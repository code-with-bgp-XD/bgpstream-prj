...
3. C++ 分析中层 + 处理器插件
   分析中层按配置切片，优先读取已经生成的 `.bgpcache`，按插件声明的字段投影为 `BGPMessage` 批次并交给上层处理器。默认情况下数据缺失或错误会终止；也可以通过 `analysis.chunk_data_failure_action` 让分析模式下载并解析当前 chunk 中缺失或损坏的数据。`analysis.persist_realtime_parsed_cache` 决定运行中解析出的结果是否永久保存。
...
3. `./manage.sh run` 使用 `analysis` 区块：
   ...
   - 通过 `analysis-plan` 进度条显示资源发现与分片路径规划；严格模式会在处理消息前确认计划中的原始 MRT 均已下载；
   ...
   - `chunk_data_failure_action=download_and_parse` 时，在每个 chunk 开始前下载缺失的原始 MRT 并完整校验解析缓存；缺失、损坏和过期的缓存会按 `persist_realtime_parsed_cache` 选择永久重建或仅本次直接解析；恢复失败则终止；
...
- `cpp/include/bgpstream_runner/chunk_engine.h`
  ...
  - 按配置在当前 chunk 内下载缺失的原始 MRT，并修复解析缓存
...
缓存目录固定为 `cache.output_dir`，程序不会自动淘汰文件。默认的 `analysis.chunk_data_failure_action=stop` 会在原始 MRT 缺失或已有解析缓存错误时失败；单纯缺少解析缓存仍由 `parse_on_cache_miss` 决定终止或现场解析。设为 `download_and_parse` 后，分析程序会在当前 chunk 开始处理消息前下载其中缺失的原始 MRT，并完整读取、校验已有 `.bgpcache`。缺失、损坏及过期的缓存会进入恢复流程：`persist_realtime_parsed_cache=true` 时原子重建为永久缓存，设为 `false` 时绕过错误缓存并直接解析原始 MRT，不写入新的解析缓存。该模式优先于 `parse_on_cache_miss`；下载、校验或解析失败时仍会终止。它不会静默覆盖已经存在但本身无法解析的原始 MRT。
...
```json
{
  "analysis": {
    ...
    "chunk_data_failure_action": "stop",
    "chunk_size": 1,
    "chunk_unit": "day",
    "limit": -1,
...
- `--chunk-data-failure-action stop|download-and-parse`
...
- `analysis.parse_on_cache_miss`
  布尔值，默认 `false`。为 `false` 时，所需 `.bgpcache` 缺失会立即终止分析；为 `true` 时，程序用 libBGPStream 现场解析已经下载的原始 MRT。此开关本身不允许自动下载，也不把损坏、过期或读取中失败的已有缓存静默切换为现场解析。

- `analysis.persist_realtime_parsed_cache`
  布尔值，默认 `false`，在 `parse_on_cache_miss=true` 的缓存缺失回退，以及 `chunk_data_failure_action=download_and_parse` 的自动恢复中生效。为 `false` 时，消息按 MRT 原始顺序直接流式交给插件，不生成永久缓存；要求严格时序的插件若发现逆序会终止运行。为 `true` 时，程序先生成经过稳定时间排序和完整校验的 `.bgpcache`，原子发布后再读取并交给插件；该缓存会跨运行保留，但仍可由 `cache-clear` 删除。生成过程中还会使用 `.bgpcache.sort.part` 和 `.bgpcache.part` 临时文件，因此应为临时文件与最终缓存预留足够磁盘空间。

- `analysis.chunk_data_failure_action`
  字符串，默认 `stop`。`stop` 在原始 MRT 缺失或已有解析缓存错误时停止；仅缺少缓存时仍遵循 `parse_on_cache_miss`。`download_and_parse` 会在每个 chunk 开始前下载缺失的原始 MRT、完整校验已有缓存，并解析缺失、损坏及过期缓存对应的原始文件，完成后才向插件交付消息。此模式优先于 `parse_on_cache_miss`，但仍服从 `persist_realtime_parsed_cache`：设为 `true` 时并行重建永久缓存，设为 `false` 时仅在本次运行中直接解析。启用后每个 chunk 的已有缓存会多执行一次完整校验读取，安全性更高，但会增加磁盘 I/O；任何下载或恢复失败都会终止运行。
